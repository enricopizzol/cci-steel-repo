...
def generate_pads():
    ...
    fe = open("estantiation.txt", "w")

    for _input in inputs:
        if _input[2] == INPUT:
            fe.write(".pin({pin}),\n".format(pin=_input[0], pin_I=_input[1]))
            fe.write(".pin_I({pin_I}),\n".format(pin=_input[0], pin_I=_input[1]))
            ...
            if int(_input[SIZE]) > 1:
                for x in range(0, int(_input[SIZE])):
                    file.write(
                        "ICP PAD_{pin}{index}_i(.PAD({pin}[{index}]),.Y({pin_I}[{index}]));\n".format(pin=_input[0],
                                                                                                      pin_I=_input[1],
                                                                                                      index=x))
        if _input[2] == OUTPUT:
            fe.write(".pin({pin}),\n".format(pin=_input[0], pin_I=_input[1]))
            ...
            if int(_input[SIZE]) > 1:
                for x in range(0, int(_input[SIZE])):
                    ...
                    file.write(
                        "BD8P PAD_{pin}{index}_o(.A({pin_I}[{index}]),.PAD({pin}[{index}]));\n".format(pin=_input[0],
                                                                                                       pin_I=_input[1],
                                                                                                       index=x))
...
def generate_estantiation():
    with open(CSV_FILENAME) as f:
        reader = csv.reader(f)
        inputs = list(tuple(line) for line in reader)
    file = open("estantiation.txt", "w")
